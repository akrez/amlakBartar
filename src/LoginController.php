...
class LoginController extends HomeController
{
    ...
    public function password()
    {
        ...
        if (!empty($_POST['password'])) {
            ...
            $db = new DataBase;
            if ($db->select("SELECT * FROM users WHERE password='$password' and id='" . $_SESSION['user_id'] . "'")) {
                $name = $db->getField("SELECT name FROM users WHERE id='" . $_SESSION['user_id'] . "'", 'name');
    ...
    public function changePass()
    {
        ...
        if (!empty($_POST['password']) && !empty($_POST['password_again'])) {
            ...
            $password = md5($password);
            $db = new DataBase;
            if ($db->change("UPDATE  users SET password='$password' WHERE id='" . $_SESSION['user_id'] . "'")) {
